...
def movement_queen(current_board, counter, sideway_counter, restart_counter, b_size):
    ...
    if parent_h != 0:
        ...
        if value <= parent_h:
            ...
            if value < parent_h:
                print("Move")
                print(child_board)
                return movement_queen(child_board, counter + 1, 0, restart_counter, b_size)
            elif value == parent_h:
                if sideway_counter >= 100:
                    ...
                    print("Restart Board")
                    print(restart_board)
                    return movement_queen(restart_board, counter + 1, 0, restart_counter + 1, b_size)

                else:
                    sideway_counter = sideway_counter + 1
                    print("Move")
                    print(child_board)
                    return movement_queen(child_board, counter + 1, sideway_counter, restart_counter, b_size)
        else:
            restart_board = np.zeros((b_size, b_size), np.dtype(int))
            restart_board = np.reshape(restart_board, (-1, b_size))
            for i in range(0, b_size):
...
class NQueenHillClimbing:
    ...
    while counter_avg > 0:
        ...
        print("Initial Board")
        print(current_board)
        steps_count, goal_test, restart_counter = movement_queen(current_board, 1, 0, 0, bsize)
        if goal_test == "F":

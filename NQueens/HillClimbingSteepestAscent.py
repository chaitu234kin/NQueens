...
def movement_queen(current_board, counter):
    ...
    if parent_h != 0:
        ...
        if value < parent_h:
            ...
            print("Move")
            print(child_board)
            return movement_queen(child_board, counter + 1)
        else:
            print("Fail")
            return counter, "F"
    else:
...
class NQueenHillClimbing:
    ...
    while counter_avg > 0:
        ...
        print("Initial Board")
        print(current_board)
        steps_count, goal_test = movement_queen(current_board, 1)
        if goal_test == "F":
    ...
    print("Rates: ")
    print("Failure rate: ", round((f_counter/total) * 100, 2), " Success rate: ", round((g_counter/total) * 100, 2), " total ", total)
